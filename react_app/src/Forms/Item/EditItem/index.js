...
export const EditItemForm = ({post}) => {
    ...
    useEffect(() => {
        const expDateParsed = `${post.expDate.toString().slice(12, 16)}-${Object.keys(monthNames).find(key => monthNames[key] === post.expDate.toString().slice(8, 11))}-${post.expDate.toString().slice(5, 7)}`;
        setExpDate(expDateParsed)
        console.log(expDate)
    }, [])
                        ...
                        {!sessionUser.isNonprofit && (
                            <>
                                ...
                                <Fieldset>
                                    ...
                                    <input id='imageUpload' style={{borderRadius: '3px', color: 'rgb(40, 166, 144)'}} type="file" accept="image/png, image/jpeg, image/jpg" value={image} onChange={updateImage} />
                                </Fieldset>
                            </>
                        )}
                        {titleErrors && (
